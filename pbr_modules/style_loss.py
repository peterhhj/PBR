...
class VGGStyleLoss(nn.Module):
    def __init__(self, device: str = "cuda") -> None:
        ...
        self.register_buffer(
            "mean",
            torch.tensor([0.485, 0.456, 0.406], device=device, dtype=torch.float32).view(1, 3, 1, 1),
        )
        self.register_buffer(
            "std",
            torch.tensor([0.229, 0.224, 0.225], device=device, dtype=torch.float32).view(1, 3, 1, 1),
        )
    ...
    def build_style_cache(self, target_image: torch.Tensor) -> Dict[str, torch.Tensor]:
        target_image = target_image if target_image.ndim == 4 else target_image.unsqueeze(0)
        with torch.no_grad():
            target_features = self.get_features(target_image)
            return {
                layer_name: self.gram_matrix(feature).detach()
                for layer_name, feature in target_features.items()
            }

    def build_content_cache(self, target_image: torch.Tensor) -> torch.Tensor:
        target_image = target_image if target_image.ndim == 4 else target_image.unsqueeze(0)
        with torch.no_grad():
            target_features = self.get_features(target_image)
        return target_features[self.content_layer].detach()
    ...
    def forward(
        ...
        target_cache = self.build_style_cache(target_image)
        return self.forward_from_cache(pred_image, target_cache, layer_weights=layer_weights)
    ...
    def forward_from_cache(
        ...
        for layer_name, pred_feature in pred_features.items():
            ...
            target_gram = target_cache[layer_name]
    ...
    def content_loss(self, pred_image: torch.Tensor, target_image: torch.Tensor) -> torch.Tensor:
        target_cache = self.build_content_cache(target_image)
        return self.content_loss_from_cache(pred_image, target_cache)

    def content_loss_from_cache(self, pred_image: torch.Tensor, target_cache: torch.Tensor) -> torch.Tensor:
        pred_features = self.get_features(pred_image)
        return F.l1_loss(pred_features[self.content_layer], target_cache)
