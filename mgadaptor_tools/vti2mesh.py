...
def _resolve_iso_value(mesh_like, scalar_name: str, iso_value: Optional[float], iso_percentile: float) -> float:
    ...
    if not (0.0 <= float(iso_percentile) <= 100.0):
        raise ValueError(
            f"--iso_percentile must be in [0, 100], but got {iso_percentile}. "
            "If you want to pass an absolute threshold, use --iso_value instead."
        )
    ...
    percentile = float(iso_percentile) / 100.0
    return minimum + (maximum - minimum) * percentile


def _extract_largest_component(surface):
    try:
    ...
    if hasattr(surface, "extract_largest"):
        return surface.extract_largest()

    connected = surface.connectivity()
    region_key = None
    for candidate in ("RegionId", "RegionID", "region_id"):
        if candidate in getattr(connected, "array_names", []):
            region_key = candidate
            break
    if region_key is None:
        raise RuntimeError(
            "Failed to extract the largest connected component: this PyVista version "
            "does not support the expected connectivity APIs."
        )

    import numpy as np

    regions = connected.cell_data[region_key]
    unique_ids, counts = np.unique(regions, return_counts=True)
    largest_region = unique_ids[counts.argmax()]
    return connected.threshold(
        [float(largest_region) - 0.5, float(largest_region) + 0.5],
        scalars=region_key,
    )
...
def convert_vti_to_mesh(
    ...
    *,
    ...
    if largest_component:
        surface = _extract_largest_component(surface)
    if fill_holes > 0:
