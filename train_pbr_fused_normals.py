...
def metal_brightness_floor_loss(
    ...
    # Use channel-average brightness instead of luminance so the optimizer
    # cannot satisfy the floor mainly by pushing the green channel.
    brightness = basecolor.mean(dim=-1, keepdim=True)
    penalty = F.relu(float(target_luminance) - brightness)
...
def metal_style_mean_anchor_loss(
    basecolor: torch.Tensor,
    metallic: torch.Tensor,
    ...
    if metallic_weights.sum() < 1.0:
        return basecolor.new_tensor(0.0)
    weighted_mean = (basecolor * metallic_weights).sum(dim=0) / metallic_weights.sum().clamp(min=1.0)
    return F.l1_loss(weighted_mean, target_mean.to(device=basecolor.device, dtype=basecolor.dtype))
...
def train_pbr_stylization_fused_normals(
    ...
    metal_style_mean_weight: float = 0.08,
    ...
    for iteration in progress_bar:
        ...
        metal_style_mean_loss = pred_basecolor.new_tensor(0.0)
        ...
        if material_preset == "metal":
            ...
            metal_style_mean_loss = metal_style_mean_anchor_loss(
                pred_basecolor,
                pred_metallic,
                target_mean=style_target_stats["mean"],
                metallic_threshold=metal_region_threshold,
            )
        ...
        total_loss = total_loss + metal_style_mean_weight * transfer_progress * metal_style_mean_loss
        ...
        loss_row = {
            ...
            "MSA": float(metal_style_mean_loss.item()),
    ...
    metadata = {
        ...
        "metal_style_mean_weight": metal_style_mean_weight,
...
def main() -> None:
    ...
    parser.add_argument("--metal_style_mean_weight", type=float, default=0.08)
    ...
    train_pbr_stylization_fused_normals(
        ...
        metal_style_mean_weight=args.metal_style_mean_weight,
